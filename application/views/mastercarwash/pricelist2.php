         ...
         <div class="col-xs-12">
             <div class="box" id="print-section">
                 <div class="box-header">
                     <h3 class="box-title"><i class="fa fa-arrow-circle-right" aria-hidden="true"></i>Price List </h3>
                 </div>
                 <div class="box-body">
             ...
             <form opd="form" id="accounts_form" onsubmit="return false;" type="multipart" autocomplete="off">
                 ...
                 <div class="modal-body">
                     <div class="form-group">
                         <?php
                            $data = array('type' => 'hidden', 'name' => 'id_ref', 'id' => 'id_ref', 'placeholder' => '');
                            echo form_input($data);
                            echo form_label('Nama :');
                            $data = array('class' => 'form-control input-lg', 'type' => 'text', 'name' => 'label', 'id' => 'label', 'placeholder' => 'e.g Avanza / City Card / truck', 'reqiured' => '');
                            echo form_input($data);
                            ?>
                     </div>
                     <div class="form-group">
 ...
 <script>
     ...
     $(document).ready(function() {
         ...
         var BankModal = {
             ...
             'id_ref': $('#accounts_modal').find('#id_ref'),
         ...
         function renderBanks(data) {
             ...
             Object.values(data).forEach((d) => {
                 var editButton = `
                 <button type="button" class="edit btn btn-primary  btn-icon" data-id='${d['id_ref']}' title="Edit"><i class='la la-pencil-alt'></i></button>
                 `;
                 var deleteButton = `
                 <button  type="button" class="delete btn btn-warning btn-icon" data-id='${d['id_ref']}' title="Delete"><i class='la la-trash'></i></button>
                 `;
         ...
         FDataTable.on('click', '.edit', function() {
             ...
             BankModal.id_ref.val(currentData['id_ref']);
         ...
         FDataTable.on('click', '.delete', function() {
             ...
             Swal.fire(swalDeleteConfigure).then((result) => {
                 ...
                 $.ajax({
                     url: "<?= base_url('MasterCarwash/actPriceList2/del') ?>",
                     'type': 'post',
                     data: {
                         'id_ref': currentData
                     },
         ...
         BankModal.form.submit(function(event) {
             ...
             var url = "<?= site_url('MasterCarwash/actPriceList2/') ?>";
             url += isAdd ? "add" : "edit";
