...
class UserModel extends CI_Model
{
    public function getNotification($filter = [])
    {
        ...
        $this->db->join('mp_multipleroles', 'notification.to_role = mp_multipleroles.menu_Id', 'LEFT');
        $this->db->where('mp_multipleroles.user_id = "' . $this->session->userdata('user_id')['id'] . '" OR notification.to_user = "' . $this->session->userdata('user_id')['id'] . '"');
